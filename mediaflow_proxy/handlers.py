...
async def get_manifest(
    ...
    """
    # Use FFmpeg remuxing if enabled
    # Legacy Mode Enforced: Stream Remuxing disabled globally.
    # if settings.ffmpeg_remux:
    #    return await handle_ffmpeg_request(request, manifest_params, proxy_headers)
...
async def handle_ffmpeg_request(
    request: Request,
    manifest_params: MPDManifestParams,
    proxy_headers: ProxyRequestHeaders,
) -> Response:
    """
    Handles a request to transcode an MPD stream using FFmpeg.
    """
    from .utils.ffmpeg_manager import ffmpeg_manager
    from .utils.http_utils import get_original_scheme

    # Extract DRM parameters
    clean_url = manifest_params.destination
    key_id = manifest_params.key_id
    key = manifest_params.key

    # Extract KID:KEY format if present
    clearkey_param = None
    if key_id and key:
        clearkey_param = f"{key_id}:{key}"

    # Start FFmpeg stream
    stream_id = await ffmpeg_manager.get_stream(clean_url, proxy_headers.request, clearkey=clearkey_param)

    if not stream_id:
        raise HTTPException(status_code=502, detail="Failed to start FFmpeg transcoding")

    # Construct the local URL for the playlist
    scheme = get_original_scheme(request)
    host = request.headers.get("host")
    local_url = f"{scheme}://{host}/proxy/mpd/ffmpeg-segment/{stream_id}/index.m3u8"

    # Add api_password if present
    api_password = request.query_params.get("api_password")
    if api_password:
        local_url += f"?api_password={api_password}"

    # Return a Master Playlist that points to the FFmpeg generated playlist
    master_playlist = (
        "#EXTM3U\n"
        "#EXT-X-VERSION:3\n"
    ...
    return Response(content=master_playlist, media_type="application/vnd.apple.mpegurl")
