...
- `ENABLE_HLS_PREBUFFER`: Optional. Enables HLS pre-buffering for improved streaming performance. Default: `false`. Pre-buffering downloads upcoming segments ahead of playback to reduce buffering.
...
- `ENABLE_DASH_PREBUFFER`: Optional. Enables DASH pre-buffering for improved streaming performance. Default: `false`. Pre-buffering downloads upcoming segments ahead of playback to reduce buffering.
...
- `REMUX_TO_TS`: Optional. Enable FFmpeg segment remuxing for MPD streams (converts fMP4 to MPEG-TS). Default: `false`. Enable this to improve compatibility with players that don't support fMP4/CMAF (like Stremio, Samsung Tizen, LG WebOS).
